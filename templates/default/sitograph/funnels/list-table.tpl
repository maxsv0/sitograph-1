...
{elseif $itemFieldID === "status"}
<td>
{if $itemField}
        <span class="text-success">{$t["yes"]}</span>
    {else}
        <span class="text-danger">{$t["no"]}</span>
        <p><a href="{$lang_url}{$admin_url}?section={$admin_section}&table={$admin_table}&send_to_crm={$item.id}&p={$admin_list_page}" class="btn btn-primary btn-sm">{$t["btn.funnels.send"]} <span class="glyphicon glyphicon-new-window"></span></a></p>
    {/if}
</td>
